...
from gplus_trees.display import collect_leaf_keys, print_pretty  # noqa: E402, F401
from gplus_trees.tree_stats import Stats, _get_capacity, gtree_stats_  # noqa: E402, F401
...
class _SplitContext:
    ...
    __slots__ = ("left_parent", "left_x_entry", "right_entry", "right_parent")

    def __init__(
        self,
        right_parent: GPlusTreeBase,
        right_entry: Entry | None,
        left_parent: GPlusTreeBase,
        left_x_entry: Entry | None,
    ) -> None:
...
class GPlusNodeBase:
    ...
    __slots__ = ("next", "rank", "right_subtree", "set")

    # set by factory
    SetClass: type[AbstractSetDataStructure]
    TreeClass: type[GPlusTreeBase]

    def __init__(self, rank: int, set: AbstractSetDataStructure, right: GPlusTreeBase | None = None) -> None:
        if rank <= 0:
...
class GPlusTreeBase(AbstractSetDataStructure):
    ...
    def __init__(self, node: GPlusNodeBase | None = None):
        self.node: GPlusNodeBase | None = node
    ...
    @staticmethod
    def _find_in_leaf(leaf_node: GPlusNodeBase, key: int, with_next: bool) -> tuple[Entry | None, Entry | None]:
    ...
    def _handle_rank_mismatch(
        self, cur: GPlusTreeBase, parent: GPlusTreeBase, p_next: Entry, rank: int
    ) -> GPlusTreeBase:
    ...
    def _update_existing_item(
        self,
        cur: GPlusTreeBase,
        new_item: InternalItem | LeafItem,
    ...
    def _insert_new_item(
        ...
        while True:
            ...
            new_tree = self._handle_right_split(right_split, right_insert, is_leaf, node, next_entry, ctx)
            next_cur = self._handle_left_split(cur, left_split, next_entry, is_leaf, ctx)
    ...
    def _handle_left_split(
        ...
        new_subtree = next_entry.left_subtree if next_entry else cur.node.right_subtree
        if ctx.left_x_entry is not None:
    ...
    @staticmethod
    def _link_split_leaves(left_leaf: GPlusTreeBase, right_leaf: GPlusTreeBase) -> None:
