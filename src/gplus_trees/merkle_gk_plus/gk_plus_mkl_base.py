...
class MerkleGKPlusNodeBase(GKPlusNodeBase):
    """
    Extension of GKPlusNodeBase that adds Merkle tree functionality.
...
class MerkleGKPlusTreeBase(GKPlusTreeBase):
    """
    Extension of GKPlusTreeBase that adds Merkle tree functionality.
    ...
    def get_root_hash(self) -> Optional[bytes]:
        ...
        if isinstance(self.node, MerkleGKPlusNodeBase):
    ...
    def _invalidate_all_hashes(self):
        ...
        for entry in self.node.set:
            if entry.left_subtree and not entry.left_subtree.is_empty():
                if isinstance(entry.left_subtree, MerkleGKPlusTreeBase):
        ...
        if self.node.right_subtree and not self.node.right_subtree.is_empty():
            if isinstance(self.node.right_subtree, MerkleGKPlusTreeBase):
    ...
    def insert(self, x: Item, rank: int) -> 'MerkleGKPlusTreeBase':
        ...
        Returns:
            MerkleGKPlusTreeBase: The updated tree
        """
    ...
    def _build_inclusion_proof(self, key: int, proof: list) -> bool:
        ...
        for entry in node.set:
            ...
            # Add sibling hashes
            if entry.left_subtree and not entry.left_subtree.is_empty():
                if isinstance(entry.left_subtree.node, MerkleGKPlusNodeBase):
