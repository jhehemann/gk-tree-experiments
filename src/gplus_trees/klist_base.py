...
class KListNodeBase:
    ...
    __slots__ = ("entries", "keys", "next", "real_keys")
    ...
    def __init__(self):
        ...
        self.next: KListNodeBase | None = None
    ...
    def insert_entry(
        self,
        entry: Entry,
    ) -> tuple[Entry | None, bool, Entry | None]:
        """
    ...
    def retrieve_entry(self, key: int) -> tuple[Entry | None, Entry | None, bool]:
        """
    ...
    def get_by_offset(self, offset: int) -> tuple[Entry, Entry | None, bool]:
        """
...
class KListBase(AbstractSetDataStructure):
    ...
    __slots__ = ("_bounds", "_nodes", "_prefix_counts_real", "_prefix_counts_tot", "head", "tail")
    ...
    def insert_entry(self, entry: Entry, rank: int | None = None) -> tuple["KListBase", bool, Entry | None]:
    ...
    def retrieve(self, key: int, with_next: bool = True) -> tuple[Entry | None, Entry | None]:
    ...
    def _locate_split_node(self, key: int) -> tuple[KListNodeBase, KListNodeBase | None, KListNodeBase | None] | None:
    ...
    def _bisect_node_entries(self, split_node: KListNodeBase, key: int) -> tuple:
        ...
        return (left_entries, left_keys, left_real_keys, right_entries, right_keys, right_real_keys, left_subtree)

    def split_inplace(self, key: int) -> tuple["KListBase", Optional["GPlusTreeBase"], "KListBase", Entry | None]:
        ...
        (left_entries, left_keys, left_real_keys, right_entries, right_keys, right_real_keys, left_subtree) = (
            self._bisect_node_entries(split_node, key)
        )
    ...
    def _rebalance_for_compaction(self, klist: "KListBase", start_node: KListNodeBase | None = None) -> None:
    ...
    def print_structure(self, indent: int = 0, depth: int = 0, max_depth: int = 2):
        ...
        while node:
            ...
            for entry in node.entries:
                result.append(f"{' ' * indent}• {entry.item!s}")
    ...
    def check_invariant(self) -> None:
        ...
        assert (self.head is None and self.tail is None) or (self.tail is not None and self.tail.next is None), (
        ...
        while node is not None:
            ...
            for i in range(1, len(node.entries)):
                ...
                assert k0 <= k1, f"Intra-node sort order violated in node {node}: {k0} > {k1}"

            # 2b) Parallel lists mirror entries
            expected_keys = [e.item.key for e in node.entries]
            assert node.keys == expected_keys, (
                f"keys list out of sync in node {nodes_seen}: keys={node.keys}, expected={expected_keys}"
            )
            expected_real = [k for k in expected_keys if k >= 0]
            assert node.real_keys == expected_real, (
                f"real_keys list out of sync in node {nodes_seen}: real_keys={node.real_keys}, expected={expected_real}"
            )
            ...
            if previous_last_key is not None and node.entries:
                ...
                assert previous_last_key < first_key, (
                    f"Inter-node invariant violated between nodes: {previous_last_key} >= {first_key}"
                )
