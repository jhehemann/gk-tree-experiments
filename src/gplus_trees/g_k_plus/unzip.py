...
class GKPlusUnzipMixin:
    ...
    def unzip(self, key: int):
        ...
        # Handle leaf nodes
        if is_leaf:
            return self._handle_leaf_unzip(left_set, left_subtree_of_key, right_set, next_entry, left_next)

        # Handle non-leaf nodes

        return self._handle_nonleaf_unzip(
            key, left_set, left_subtree_of_key, right_set, next_entry, right_subtree, rank
        )

    def _retrieve_next(self, key: int) -> Entry | None:
    ...
    def _handle_nonleaf_unzip(self, key, left_set, left_subtree_of_key, right_set, next_entry, right_subtree, rank):
        ...
        if left_subtree_of_key:
            return self._handle_key_subtree_case(
                key, left_set, left_subtree_of_key, right_set, next_entry, right_subtree, rank
            )
        ...
        return self._handle_no_key_subtree_case(key, left_set, left_count, right_set, next_entry, right_subtree, rank)
    ...
    def _handle_key_subtree_case(self, key, left_set, left_subtree_of_key, right_set, next_entry, right_subtree, rank):
        ...
        key_subtree, right_return, updated_next_entry = self._unzip_right_side(
            key, next_entry, right_subtree, right_set, rank
        )
        ...
        if next_entry:
            if next_entry.left_subtree:
                ...
                next_entry.left_subtree = (
                    r_leftmost_subtree if r_leftmost_subtree and not r_leftmost_subtree.is_empty() else None
                )
    ...
    def _unzip_right_side(self, key, next_entry, right_subtree, right_set, rank):
        ...
        if next_entry:
            _left, key_subtree, r_leftmost_subtree, new_next = next_entry.left_subtree.unzip(key)
            next_entry.left_subtree = (
                r_leftmost_subtree if r_leftmost_subtree and not r_leftmost_subtree.is_empty() else None
            )
            right_return = self.lift(right_set, right_subtree, rank)
            ...
            if right_subtree:
                _left, key_subtree, r_leftmost_subtree, new_next = right_subtree.unzip(key)
    ...
    def lift(self, set: AbstractSetDataStructure | None, right: GKPlusTreeBase, rank: int) -> GKPlusTreeBase:
        """
