...
from gplus_trees.gplus_tree_base import GPlusNodeBase, GPlusTreeBase
...
def make_gplustree_classes(
    K: int,
) -> tuple[type[GPlusTreeBase], type[GPlusNodeBase], type[KListBase], type[KListNodeBase]]:
    """
    ...
    KListNodeK = type(f"KListNode_K{K}", (KListNodeBase,), {"CAPACITY": K, "__slots__": KListNodeBase.__slots__})

    # 2) KList class points at the node class
    KListK = type(f"KList_K{K}", (KListBase,), {"KListNodeClass": KListNodeK, "__slots__": KListBase.__slots__})

    # 3) Create GPlusTreeK with a forward reference (to be set later)
    GPlusTreeK = type(f"GPlusTree_K{K}", (GPlusTreeBase,), {"SetClass": KListK, "__slots__": GPlusTreeBase.__slots__})

    # 4) GPlus-node class uses the KListK and references the already created GPlusTreeK
    GPlusNodeK = type(
        f"GPlusNode_K{K}",
        (GPlusNodeBase,),
        {"SetClass": KListK, "TreeClass": GPlusTreeK, "__slots__": GPlusNodeBase.__slots__},
    )

    # 5) Set NodeClass on GPlusTreeK now that GPlusNodeK exists
    GPlusTreeK.NodeClass = GPlusNodeK
