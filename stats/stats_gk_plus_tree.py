...
def repeated_experiment(size: int, repetitions: int, K: int, l_factor: float, adversarial_dim: int) -> None:
    """
    ...
    if adversarial_dim:
        ...
        keys = load_adversarial_keys_from_file(key_count=size, capacity=K, dim_limit=adversarial_dim)
    ...
    for _ in range(repetitions):
        ...
        if adversarial_dim:
            ...
            for item, rank in zip(items, ranks, strict=False):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--l_factor", type=float, default=1.0, help="l_factor for the tree.")
    ...
    for n in sizes:
        for K in Ks:
            ...
            logger.info("")
            logger.info(
                f"---------------- NOW RUNNING EXPERIMENT: n = {n}, K = {K}, l_factor: {l_factor}, repetitions = {repetitions}, adversarial_dim = {adversarial_dim} ----------------"
            )
            t0 = time.perf_counter()
            # If you want to use adversarial or dim, you need to modify repeated_experiment and tree creation accordingly.
            repeated_experiment(
                size=n, repetitions=repetitions, K=K, l_factor=l_factor, adversarial_dim=adversarial_dim
            )
