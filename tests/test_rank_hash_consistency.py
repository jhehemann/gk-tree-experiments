...
class TestRankHashConsistency(unittest.TestCase):
            ...
            for dim in self.test_dimensions:
                with self.subTest(key=key, dim=dim):
                    ...
                    for target_dim in range(2, dim + 1):
                        digest2 = hashlib.sha256(
                            digest2 + target_dim.to_bytes(32, 'big')
                        ).digest()
                    
                    self.assertEqual(digest1, digest2,
                                   f"Digest calculation inconsistency for key={key}, dim={dim}")
    ...
    def test_negative_keys_handled_correctly(self):
        ...
        for neg_key, pos_key in zip(negative_keys, positive_keys):
            for k in self.test_k_values:
                for dim in self.test_dimensions:
    ...
    def test_rank_calculation_deterministic(self):
        ...
        self.assertTrue(all(r == ranks[0] for r in ranks),
                       f"Rank calculations should be deterministic: {ranks}")
    ...
    def test_different_keys_different_distributions(self):
        ...
        self.assertGreater(len(unique_ranks), 1,
                          "Different keys should produce different ranks")
        
        # All ranks should be valid
        self.assertTrue(all(r >= 1 for r in ranks),
