...
class TestGKPlusSplitInplace(GKPlusTreeTestCase):
    ...
    def _run_split_case(self, keys, rank_combo, split_key,
        ...
        # assertions
        self.assertIs(tree_copy, left, msg)
    ...
    def test_split_single_node_tree(self):
        ...
        base_tree, _ = base_tree.insert(item, rank=1)
    ...
    def test_split_leaf_node_with_multiple_items(self):
        ...
        for key in keys:
            tree, _ = tree.insert(Item(key, "val"), rank=1)
    ...
    def test_split_with_complex_tree(self):
        ...
        logger.debug(f"Expected left: {exp_left}")
        logger.debug(f"Expected right: {exp_right}")
    ...
    def test_split_leaf_with_left_subtrees_higher_dim(self):
        ...
        keys = [100, 200, 300, 400, 500]
        for key in keys:
            base_tree, _ = base_tree.insert(Item(key, "val"), rank=1)
    ...
    def test_split_with_node_collapsing(self):
        ...
        ranks = [1, 2, 2, 3, 1, 2]
        for key, rank in zip(keys, ranks):
            tree, _ = tree.insert(Item(key, "val"), rank=rank)
    ...
    #     k = 4
    #     num_items = 7
    #     repetitions = 1000  # Number of subtests with different random seeds/split keys

    #     for i in tqdm(range(repetitions), desc="Running split tests with random items", unit="trial"):
