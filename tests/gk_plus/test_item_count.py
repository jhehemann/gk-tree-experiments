...
class TestGKPlusTreeItemCountTracking(GKPlusTreeTestCase):
    def test_empty_tree(self):
        ...
        self.assertIsNone(tree.item_cnt, "item_cnt should be None before triggering item_count()")
        self.assertEqual(tree.item_count(), 0, "Item count should be 0 after triggering item_count()")
        self.assertEqual(tree.item_cnt, 0, "item_cnt should have changed to 0 after triggering item_count()")
    ...
    def test_empty_tree_insertion_rank_1(self):
        ...
        self.assertIsNone(tree.item_cnt, "item_cnt should be None before triggering item_count()")
        self.assertEqual(tree.item_count(), 2, "Item count should be 2 after inserting one item (1 dummy + 1 item)")
        self.assertEqual(tree.item_cnt, 2, "item_cnt should be 2 after item_count() is called")
    ...
    def test_leaf_insertion(self):
        ...
        self.assertIsNone(tree.item_cnt, "item_cnt should be None before triggering item_count()")
        self.assertEqual(tree.item_count(), 4, "Item count should be 4 after inserting three items (1 dummy + 3 items)")
        self.assertEqual(tree.item_cnt, 4, "item_cnt should be 4 after item_count() is called")
    ...
    def test_internal_insertion_long_path(self):
        ...
        tree = self.tree_k4
        rank_lists = [[1, 4, 2, 1, 3, 2], [1, 2, 1, 1, 1, 1]]
        ...
        while cur.node.right_subtree is not None:
            self.assertIsNone(
                cur.item_cnt, f"item_cnt should be None on path to inserted item at node {print_pretty(cur.node.set)}"
            )
            for entry in cur.node.set:
                if entry.item.key == insert_item.key and entry.left_subtree is not None:
                    self.assertIsNone(
                        entry.left_subtree.item_cnt,
    ...
    def test_insertion_triggering_klist_to_tree(self):
        ...
        self.assertEqual(
            len(keys) + expanded_leafs + 1,
            tree.item_count(),
            f"Tree size should be {len(keys) + expanded_leafs + 1} after inserting {len(keys)} items with max dimension {max_dim} and expanded leaf count {expanded_leafs}. Leaf keys: {expected_keys}",
        )
    ...
    def test_large_tree_size(self):
        ...
        # Insert all items
        for inserted_count, key in enumerate(unique_keys, 1):
        ...
        for i, key in enumerate(range(4, 11), start=1):
            with self.subTest(f"Insert item with key {key * 500}"):
                ...
                if logger.isEnabledFor(logging.DEBUG):
                    ...
                    logger.debug(
                        f"Tree size should be {i + expanded_leafs + 1} after inserting {i} items with max dimension {max_dim} and expanded leaf count {expanded_leafs}. Leaf keys: {expected_keys}"
                    )
                ...
                self.assertEqual(
                    expected_real_keys, real_keys, f"Tree should contain keys {expected_real_keys} after {i} insertions"
                )
            if logger.isEnabledFor(logging.DEBUG):
    ...
    def test_large_tree_size_various_seeds_case_81(self):
        """Test size is correctly maintained in a larger tree with random insertions using multiple seeds"""
        logger.debug("Testing large tree size with seed 81")
        tree = create_gkplus_tree(K=4)
        random.seed(81)  # For reproducibility
