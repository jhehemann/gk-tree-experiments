...
class TestGKPlusTreeZip(TreeTestCase):
    ...
    def _validate_tree_after_zip(self, tree: GKPlusTreeBase, expected_keys: list[int], k: int = 4):
        ...
        self.assertEqual(
            expected_keys_sorted, actual_keys_sorted, f"Expected keys {expected_keys_sorted}, got {actual_keys_sorted}"
        )
    ...
    def test_zip_different_ranks_left_rank_smaller_right_has_left_subtree(self):
        """Test zipping when left has smaller rank than right."""
        k = 4
        rank_lists = [[1, 1, 1, 1, 2], [1, 1, 1, 1, 2]]
        keys = self.find_keys_for_rank_lists(rank_lists, k=k)
        keys_small = keys[:1]
        logger.debug(f"Keys for small tree: {keys_small}")
        ranks_small = rank_lists[0][:1]
    ...
    def test_zip_item_counts(self):
        ...
        tree_1.item_count()
        tree_2.item_count()
        tree_1.real_item_count()
        tree_2.real_item_count()

        result.get_expanded_leaf_count()
        len(keys_small) + len(keys_large)
    ...
    def test_zip_specific_case_a(self):
        ...
        for key, rank in zip(keys1, ranks1, strict=False):
        ...
        self.assertEqual(expected_keys, actual_keys, f"Expected keys: {expected_keys}\nActual keys: {actual_keys}")

        # Verify tree size
        self.assertEqual(result.real_item_count(), len(expected_keys), "Tree real_item_count mismatch")
    ...
    def test_zip_random_trees_varying_sizes_case_a(self):
        ...
        # Sort keys and ranks
        keys_small, ranks_small = zip(
            *sorted(zip(keys_small, ranks_small, strict=False), key=lambda x: x[0]), strict=False
        )
        keys_large, ranks_large = zip(
            *sorted(zip(keys_large, ranks_large, strict=False), key=lambda x: x[0]), strict=False
        )
