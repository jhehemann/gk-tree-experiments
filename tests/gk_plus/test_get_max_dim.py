...
class TestGKPlusDimensionTracking(GKPlusTreeTestCase):
    def test_tree_k4(self):
        ...
        with self.subTest("Insert items below expansion threshold 4"):
            ...
            for i in range(3):
                ...
                self.assertTrue(inserted, f"Item with key {key} should be inserted successfully")
        ...
        with self.subTest("Insert item to trigger expansion"):
            # Insert one more item to trigger expansion
            tree, inserted, _ = tree.insert(self.make_item(keys[3]), rank=rank_lists[0][3])
            self.assertTrue(inserted, f"Item with key {keys[3]} should be inserted successfully")
            max_dim = tree.get_max_dim()
            self.assertEqual(max_dim, 2, "Maximum tree dimension should increase to 2 after expansion")
            expanded_leafs = tree.get_expanded_leaf_count()
            expected_keys = [entry.item.key for entry in tree]
            self.assertEqual(
                tree.item_count(),
                6,
                "Tree size should be 6. 4 Real items + 2 dummy items for each of the 2 dimensions.",
            )
            ...
            real_keys = [entry.item.key for entry in tree.iter_real_entries()]
            self.assertEqual(
                expected_real_keys, real_keys, f"Tree should contain keys {expected_real_keys} after expansion"
            )

            self.assertEqual(tree.get_max_dim(), 2, "Max dimension should be 2 after inserting 5 items")
