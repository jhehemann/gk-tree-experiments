...
class TestInsertInTree(TreeTestCase):
    def tearDown(self):
        ...
        self.assertFalse(self.tree.node.set.is_empty(), "Root set must not be empty")
...
class TestInsertInEmptyTree(TestInsertInTree):
    def test_insert_rank_1_creates_single_node(self):
        ...
        self._assert_leaf_node_properties(root, [DUMMY_ITEM, item])
        ...
        self.assertIsNone(next(leaf_iter, None), "There should be only one leaf node")
    ...
    def test_insert_rank_gt_1_creates_root_and_two_leaves(self):
        ...
        with self.subTest("root"):
            _, replica = self._assert_internal_node_properties(root, [DUMMY_ITEM, self.get_replica_from_key(key)], rank)

        with self.subTest("left leaf"):
            t_left_leaf = replica.left_subtree
            self._assert_leaf_node_properties(t_left_leaf.node, [DUMMY_ITEM])
            self.assertIs(t_left_leaf.node.next, root.right_subtree, "Left leaf should point to right leaf")

        with self.subTest("right leaf"):
            t_right_leaf = root.right_subtree
            self._assert_leaf_node_properties(t_right_leaf.node, [item])
            self.assertIsNone(t_right_leaf.node.next, "Right leaf should have no next pointer")
...
class TestInsertInNonEmptyTreeLeaf(TestInsertInTree):
    def setUp(self):
        ...
        self.rank_map = {key: rank for key, rank in zip(keys, ranks, strict=False)}
        for k in keys:
    ...
    def tearDown(self):
        self.assertIsNone(self.tree.node.next, "Leaf should have no next pointer")
    ...
    def test_insert_lowest_key(self):
        ...
        self._assert_leaf_node_properties(self.tree.node, [DUMMY_ITEM, item, self.item_map[2], self.item_map[4]])
    ...
    def test_insert_highest_key(self):
        ...
        self._assert_leaf_node_properties(self.tree.node, [DUMMY_ITEM, self.item_map[2], self.item_map[4], item])
    ...
    def test_insert_middle_key(self):
        ...
        self._assert_leaf_node_properties(self.tree.node, [DUMMY_ITEM, self.item_map[2], item, self.item_map[4]])
...
class TestInsertInNonEmptyTreeGTMaxRankCreatesRoot(TestInsertInTree):
    ...
    def test_insert_lowest_key(self):
        ...
        with self.subTest("root"):
            _, replica = self._assert_internal_node_properties(
                self.tree.node, [DUMMY_ITEM, self.get_replica_from_key(key)], rank
            )
        with self.subTest("left leaf"):
            t_left_leaf = replica.left_subtree
            self._assert_leaf_node_properties(t_left_leaf.node, [DUMMY_ITEM])
            self.assertIs(t_left_leaf.node.next, self.tree.node.right_subtree, "Left leaf should point to right leaf")
        with self.subTest("right leaf"):
            t_right_leaf = self.tree.node.right_subtree
            self._assert_leaf_node_properties(t_right_leaf.node, [item, self.item_map[2], self.item_map[4]])
            self.assertIsNone(t_right_leaf.node.next, "Right leaf should have no next pointer")
    ...
    def test_insert_highest_key(self):
        ...
        with self.subTest("left leaf"):
            t_left_leaf = replica.left_subtree
            self._assert_leaf_node_properties(t_left_leaf.node, [DUMMY_ITEM, self.item_map[2], self.item_map[4]])
            self.assertIs(t_left_leaf.node.next, self.tree.node.right_subtree, "Left leaf should point to right leaf")
        with self.subTest("right leaf"):
            t_right_leaf = self.tree.node.right_subtree
            self._assert_leaf_node_properties(t_right_leaf.node, [item])
    ...
    def test_insert_middle_key(self):
        ...
        with self.subTest("left leaf"):
            t_left_leaf = replica.left_subtree
            self._assert_leaf_node_properties(t_left_leaf.node, [DUMMY_ITEM, self.item_map[2]])
            self.assertIs(t_left_leaf.node.next, self.tree.node.right_subtree, "Left leaf should point to right leaf")
        with self.subTest("right leaf"):
            t_right_leaf = self.tree.node.right_subtree
            self._assert_leaf_node_properties(t_right_leaf.node, [item, self.item_map[4]])
            self.assertIsNone(t_right_leaf.node.next, "Right leaf should have no next pointer")
...
class TestInsertInNonEmptyTreeRankGT1(TestInsertInTree):
    ...
    def test_insert_lowest_key_splits_leaf(self):
        ...
        with self.subTest("root"):
            self._assert_internal_node_properties(
                root, [DUMMY_ITEM, self.get_replica_from_key(key), self.get_replica_from_key(4)], rank
            )

        with self.subTest("leaf 1"):
            leaf_1 = root_entries[1].left_subtree.node
            self._assert_leaf_node_properties(leaf_1, [DUMMY_ITEM])
            self.assertIs(leaf_1.next, root_entries[2].left_subtree, "Leaf 1 should point to leaf 2 tree")

        with self.subTest("leaf 2"):
            leaf_2 = root_entries[2].left_subtree.node
            self._assert_leaf_node_properties(leaf_2, [item, self.item_map[2]])
            self.assertIs(leaf_2.next, root.right_subtree, "Leaf 2 should point to leaf 3 tree")

        with self.subTest("leaf 3"):
            leaf_3 = root.right_subtree.node
            self._assert_leaf_node_properties(leaf_3, [self.item_map[4], self.item_map[6]])
            self.assertIsNone(leaf_3.next, "Leaf 3 should have no next pointer")
    ...
    def test_insert_lowest_key_no_leaf_split(self):
        ...
        with self.subTest("root"):
            self._assert_internal_node_properties(
                root, [DUMMY_ITEM, self.get_replica_from_key(3), self.get_replica_from_key(4)], rank
            )

        with self.subTest("leaf 1"):
            leaf_1 = root_entries[1].left_subtree.node
            self._assert_leaf_node_properties(leaf_1, [DUMMY_ITEM, self.item_map[2]])
            self.assertIs(leaf_1.next, root_entries[2].left_subtree, "Leaf 1 should point to leaf 2 tree")

        with self.subTest("leaf 2"):
            leaf_2 = root_entries[2].left_subtree.node
            self._assert_leaf_node_properties(leaf_2, [item])
            self.assertIs(leaf_2.next, root.right_subtree, "Leaf 2 should point to leaf 3 tree")
    ...
    def test_insert_highest_key_splits_leaf(self):
        ...
        with self.subTest("root"):
            self._assert_internal_node_properties(
                root, [DUMMY_ITEM, self.get_replica_from_key(4), self.get_replica_from_key(key)], rank
            )
        ...
        with self.subTest("leaf 2"):
            leaf_2 = root_entries[2].left_subtree.node
            self._assert_leaf_node_properties(leaf_2, [self.item_map[4]])
            self.assertIs(leaf_2.next, root.right_subtree, "Leaf 2 should point to leaf 3 tree")

        with self.subTest("leaf 3"):
            leaf_3 = root.right_subtree.node
            self._assert_leaf_node_properties(leaf_3, [item, self.item_map[6]])
            self.assertIsNone(leaf_3.next, "Leaf 3 should have no next pointer")
    ...
    def test_insert_highest_key_no_leaf_split(self):
        ...
        with self.subTest("leaf 2"):
            leaf_2 = root_entries[2].left_subtree.node
            self._assert_leaf_node_properties(leaf_2, [self.item_map[4], self.item_map[6]])
            self.assertIs(leaf_2.next, root.right_subtree, "Leaf 2 should point to leaf 3 tree")

        with self.subTest("leaf 3"):
            leaf_3 = root.right_subtree.node
            self._assert_leaf_node_properties(leaf_3, [item])
            self.assertIsNone(leaf_3.next, "Leaf 3 should have no next pointer")
...
class TestInsertInNonEmptyTreeCollapsedLayerCreatesInternal(TestInsertInTree):
    ...
    def test_insert_lowest_key(self):
        ...
        with self.subTest("root"):
            _, r_replica = self._assert_internal_node_properties(root, [DUMMY_ITEM, self.get_replica_from_key(4)], 3)
        ...
        with self.subTest("created internal"):
            ...
            _, i_replica = self._assert_internal_node_properties(
                new_internal, [DUMMY_ITEM, self.get_replica_from_key(key)], self.insert_rank
            )

        with self.subTest("leaf 1"):
            leaf_1 = i_replica.left_subtree.node
            self._assert_leaf_node_properties(leaf_1, [DUMMY_ITEM])
            self.assertIs(leaf_1.next, new_internal.right_subtree, "Leaf 1 should point to leaf 2 tree")
        ...
        with self.subTest("leaf 3"):
            leaf_3 = root.right_subtree.node
            self._assert_leaf_node_properties(leaf_3, [self.item_map[4]])
            self.assertIsNone(leaf_3.next, "Leaf 3 should have no next pointer")
    ...
    def test_insert_highest_key(self):
        ...
        with self.subTest("created internal"):
            ...
            _, i_replica = self._assert_internal_node_properties(
                new_internal, [self.get_replica_from_key(4), self.get_replica_from_key(key)], self.insert_rank
            )

        with self.subTest("leaf 1"):
            leaf_1 = r_replica.left_subtree.node
            self._assert_leaf_node_properties(leaf_1, [DUMMY_ITEM, self.item_map[2]])
            self.assertIs(leaf_1.next, i_replica.left_subtree, "Leaf 1 should point to leaf 2 tree")

        with self.subTest("leaf 2"):
            leaf_2 = i_replica.left_subtree.node
            self._assert_leaf_node_properties(leaf_2, [self.item_map[4]])
            self.assertIs(leaf_2.next, new_internal.right_subtree, "Leaf 2 should point to leaf 3 tree")
...
class TestInsertInNonEmptyTreeRankGT2LowestKey(TestInsertInTree):
    ...
    def test_insert_lowest_key_splits_child_lowest_collapses_left_split(self):
        ...
        with self.subTest("root"):
            self._assert_internal_node_properties(
                root, [DUMMY_ITEM, self.get_replica_from_key(key), self.get_replica_from_key(8)], 3
            )

        with self.subTest("child right split (internal)"):
            c_right = root_entries[2].left_subtree.node
            self._assert_internal_node_properties(
                c_right, [self.get_replica_from_key(key), self.get_replica_from_key(4), self.get_replica_from_key(6)], 2
            )
        ...
        with self.subTest("leaf 1"):
            leaf_1 = root_entries[1].left_subtree.node
            self._assert_leaf_node_properties(leaf_1, [DUMMY_ITEM])
            self.assertIs(leaf_1.next, c_right_entries[1].left_subtree, "Leaf 1 should point to leaf 2 tree")

        with self.subTest("leaf 2"):
            leaf_2 = c_right_entries[1].left_subtree.node
            self._assert_leaf_node_properties(leaf_2, [item, self.item_map[2]])
            self.assertIs(leaf_2.next, c_right_entries[2].left_subtree, "Leaf 2 should point to leaf 3 tree")

        with self.subTest("leaf 3"):
            leaf_3 = c_right_entries[2].left_subtree.node
            self._assert_leaf_node_properties(leaf_3, [self.item_map[4]])
            self.assertIs(leaf_3.next, c_right.right_subtree, "Leaf 3 should point to leaf 4 tree")

        with self.subTest("leaf 4"):
            leaf_4 = c_right.right_subtree.node
            self._assert_leaf_node_properties(leaf_4, [self.item_map[6]])
            self.assertIs(leaf_4.next, root.right_subtree, "Leaf 4 should point to leaf 5 tree")

        with self.subTest("leaf 5"):
            leaf_4 = root.right_subtree.node
            self._assert_leaf_node_properties(leaf_4, [self.item_map[8]])
            self.assertIsNone(leaf_4.next, "Leaf 4 should have no next pointer")
    ...
    def test_insert_lowest_key_splits_child_middle(self):
        ...
        with self.subTest("child left split (internal)"):
            ...
            self._assert_internal_node_properties(c_left, [DUMMY_ITEM, self.get_replica_from_key(4)], 2)
            c_left_entries = list(c_left.set)

        with self.subTest("child right split (internal)"):
            c_right = root_entries[2].left_subtree.node
            self._assert_internal_node_properties(
                c_right, [self.get_replica_from_key(key), self.get_replica_from_key(6)], 2
            )
            c_right_entries = list(c_right.set)
        ...
        with self.subTest("leaf 1"):
            ...
            self._assert_leaf_node_properties(leaf_1, [DUMMY_ITEM, self.item_map[2]])
            self.assertIs(leaf_1.next, c_left.right_subtree, "Leaf 1 should point to leaf 2 tree")

        with self.subTest("leaf 2"):
            leaf_2 = c_left.right_subtree.node
            self._assert_leaf_node_properties(leaf_2, [self.item_map[4]])
            self.assertIs(leaf_2.next, c_right_entries[1].left_subtree, "Leaf 2 should point to leaf 3 tree")
        ...
        with self.subTest("leaf 5"):
            ...
            self._assert_leaf_node_properties(leaf_5, [self.item_map[8]])
            self.assertIsNone(leaf_5.next, "Leaf 5 should have no next pointer")
    ...
    def test_insert_lowest_key_splits_child_highest(self):
        ...
        with self.subTest("child left split (internal)"):
            c_left = root_entries[1].left_subtree.node
            self._assert_internal_node_properties(
                c_left, [DUMMY_ITEM, self.get_replica_from_key(4), self.get_replica_from_key(6)], 2
            )
            c_left_entries = list(c_left.set)
        ...
        with self.subTest("leaf 1"):
            leaf_1 = c_left_entries[1].left_subtree.node
            self._assert_leaf_node_properties(leaf_1, [DUMMY_ITEM, self.item_map[2]])
            self.assertIs(leaf_1.next, c_left_entries[2].left_subtree, "Leaf 1 should point to leaf 2 tree")

        with self.subTest("leaf 2"):
            leaf_2 = c_left_entries[2].left_subtree.node
            self._assert_leaf_node_properties(leaf_2, [self.item_map[4]])
            self.assertIs(leaf_2.next, c_left.right_subtree, "Leaf 2 should point to leaf 3 tree")

        with self.subTest("leaf 3"):
            leaf_3 = c_left.right_subtree.node
            self._assert_leaf_node_properties(leaf_3, [self.item_map[6]])
            # child right split with single replica should collapse
            self.assertIs(leaf_3.next, root_entries[2].left_subtree, "Leaf 3 should point to leaf 4 tree")

        with self.subTest("leaf 4"):
            leaf_4 = root_entries[2].left_subtree.node
            self._assert_leaf_node_properties(leaf_4, [item])
            self.assertIs(leaf_4.next, root.right_subtree, "Leaf 4 should point to leaf 5 tree")
...
class TestInsertInNonEmptyTreeRankGT2HighestKey(TestInsertInTree):
    ...
    def test_insert_highest_key_splits_child_low_collapses_left_split(self):
        ...
        with self.subTest("root"):
            self._assert_internal_node_properties(
                root, [DUMMY_ITEM, self.get_replica_from_key(1), self.get_replica_from_key(key)], 3
            )

        with self.subTest("child right split (internal)"):
            c_right = root.right_subtree.node
            self._assert_internal_node_properties(
                c_right, [self.get_replica_from_key(key), self.get_replica_from_key(3), self.get_replica_from_key(5)], 2
            )
            c_right_entries = list(c_right.set)
        ...
        with self.subTest("leaf 2"):
            leaf_2 = root_entries[2].left_subtree.node
            self._assert_leaf_node_properties(leaf_2, [self.item_map[1]])
            self.assertIs(leaf_2.next, c_right_entries[1].left_subtree, "Leaf 2 should point to leaf 3 tree")

        with self.subTest("leaf 3"):
            leaf_3 = c_right_entries[1].left_subtree.node
            self._assert_leaf_node_properties(leaf_3, [item])
            self.assertIs(leaf_3.next, c_right_entries[2].left_subtree, "Leaf 3 should point to leaf 4 tree")

        with self.subTest("leaf 4"):
            leaf_4 = c_right_entries[2].left_subtree.node
            self._assert_leaf_node_properties(leaf_4, [self.item_map[3]])
            self.assertIs(leaf_4.next, c_right.right_subtree, "Leaf 4 should point to leaf 5 tree")

        with self.subTest("leaf 5"):
            leaf_5 = c_right.right_subtree.node
            self._assert_leaf_node_properties(leaf_5, [self.item_map[5]])
            self.assertIsNone(leaf_5.next, "Leaf 5 should have no next pointer")
    ...
    def test_insert_highest_key_splits_child_mid(self):
        ...
        with self.subTest("child left split (internal)"):
            ...
            self._assert_internal_node_properties(
                c_left, [self.get_replica_from_key(1), self.get_replica_from_key(3)], 2
            )
            c_left_entries = list(c_left.set)
        ...
        with self.subTest("child right split (internal)"):
            c_right = root.right_subtree.node
            self._assert_internal_node_properties(
                c_right, [self.get_replica_from_key(key), self.get_replica_from_key(5)], 2
            )
            c_right_entries = list(c_right.set)
        ...
        with self.subTest("leaf 1"):
            leaf_1 = root_entries[1].left_subtree.node
            self._assert_leaf_node_properties(leaf_1, [DUMMY_ITEM])
            self.assertIs(leaf_1.next, c_left_entries[1].left_subtree, "Leaf 1 should point to leaf 2 tree")
        ...
        with self.subTest("leaf 3"):
            leaf_3 = c_left.right_subtree.node
            self._assert_leaf_node_properties(leaf_3, [self.item_map[3]])
            self.assertIs(leaf_3.next, c_right_entries[1].left_subtree, "Leaf 3 should point to leaf 4 tree")
    ...
    def test_insert_highest_key_splits_child_high_collapses_right_split(self):
        ...
        with self.subTest("child left split (internal)"):
            c_left = root_entries[2].left_subtree.node
            self._assert_internal_node_properties(
                c_left, [self.get_replica_from_key(1), self.get_replica_from_key(3), self.get_replica_from_key(5)], 2
            )
            c_left_entries = list(c_left.set)
        ...
        with self.subTest("leaf 2"):
            leaf_2 = c_left_entries[1].left_subtree.node
            self._assert_leaf_node_properties(leaf_2, [self.item_map[1]])
            self.assertIs(leaf_2.next, c_left_entries[2].left_subtree, "Leaf 2 should point to leaf 3 tree")

        with self.subTest("leaf 3"):
            leaf_3 = c_left_entries[2].left_subtree.node
            self._assert_leaf_node_properties(leaf_3, [self.item_map[3]])
            self.assertIs(leaf_3.next, c_left.right_subtree, "Leaf 3 should point to leaf 4 tree")

        with self.subTest("leaf 4"):
            leaf_4 = c_left.right_subtree.node
            self._assert_leaf_node_properties(leaf_4, [self.item_map[5]])
            self.assertIs(leaf_4.next, root.right_subtree, "Leaf 4 should point to leaf 5 tree")

        # child right split with single replica should collapse
        with self.subTest("leaf 5"):
            leaf_5 = root.right_subtree.node
            self._assert_leaf_node_properties(leaf_5, [item])
            self.assertIsNone(leaf_5.next, "Leaf 5 should have no next pointer")
...
class TestInsertNonemptyTreeHighCollapsingNodesRightLeft(TestInsertInTree):
    ...
    def test_insert_high_collapse_rank_3_right_rank_2_left_split_leaf(self):
        ...
        with self.subTest("root"):
            self._assert_internal_node_properties(
                root, [DUMMY_ITEM, self.get_replica_from_key(1), self.get_replica_from_key(key)], 4
            )

        with self.subTest("child rank 3 (left split of right collapsed)"):
            c_left = root_entries[2].left_subtree.node
            self._assert_internal_node_properties(
                c_left, [self.get_replica_from_key(1), self.get_replica_from_key(2)], 3
            )
            c_left_entries = list(c_left.set)
        ...
        with self.subTest("leaf 3"):
            leaf_3 = c_left.right_subtree.node
            self._assert_leaf_node_properties(leaf_3, [self.item_map[2]])
            self.assertIs(leaf_3.next, c_right_entries[1].left_subtree, "Leaf 3 should point to leaf 4 tree")

        with self.subTest("leaf 4"):
            leaf_4 = c_right_entries[1].left_subtree.node
            self._assert_leaf_node_properties(leaf_4, [item, self.item_map[4]])
            self.assertIs(leaf_4.next, c_right.right_subtree, "Leaf 4 should point to leaf 5 tree")
...
class TestInsertNonemptyTreeMidCollapsingNodesRightLeft(TestInsertInTree):
    ...
    def test_insert_mid_collapse_rank_3_right_rank_2_left_split_leaf(self):
        ...
        with self.subTest("leaf 5"):
            leaf_5 = c_right.right_subtree.node
            self._assert_leaf_node_properties(leaf_5, [self.item_map[5]])
            self.assertIs(leaf_5.next, root.right_subtree, "Leaf 5 should point to leaf 6 tree")

        with self.subTest("leaf 6"):
            leaf_6 = root.right_subtree.node
            self._assert_leaf_node_properties(leaf_6, [self.item_map[6]])
            self.assertIsNone(leaf_6.next, "Leaf 6 should have no next pointer")
...
class TestInsertNonemptyTreeHighCollapsingNodesLeftRight(TestInsertInTree):
    ...
    def test_insert_high_collapse_rank_3_left_rank_2_right_split_leaf(self):
        ...
        with self.subTest("child rank 3 (right split of left collapsed)"):
            c_right = root.right_subtree.node
            self._assert_internal_node_properties(
                c_right, [self.get_replica_from_key(key), self.get_replica_from_key(4)], 3
            )
            c_right_entries = list(c_right.set)
        ...
        with self.subTest("child rank 2 (left split of right collapsed)"):
            c_left = root_entries[2].left_subtree.node
            self._assert_internal_node_properties(
                c_left, [self.get_replica_from_key(1), self.get_replica_from_key(2)], 2
            )
            c_left_entries = list(c_left.set)
        ...
        with self.subTest("leaf 5"):
            leaf_5 = c_right.right_subtree.node
            self._assert_leaf_node_properties(leaf_5, [self.item_map[4]])
            self.assertIsNone(leaf_5.next, "Leaf 5 should have no next pointer")
...
class TestInsertNonemptyTreeMidCollapsingNodesLeftRight(TestInsertInTree):
    ...
    def test_insert_mid_collapse_rank_3_left_rank_2_right_split_leaf(self):
        ...
        with self.subTest("leaf 6"):
            leaf_6 = root.right_subtree.node
            self._assert_leaf_node_properties(leaf_6, [self.item_map[5]])
            self.assertIsNone(leaf_6.next, "Leaf 6 should have no next pointer")
...
class TestInsertNonemptyRank4TreeUnfoldRank2(TestInsertInTree):
    ...
    def test_insert_split_leaf(self):
        ...
        with self.subTest("root"):
            self._assert_internal_node_properties(root, [DUMMY_ITEM, self.get_replica_from_key(5)], 4)

        with self.subTest("rank 3 internal node"):
            internal_3 = root_entries[1].left_subtree.node
            self._assert_internal_node_properties(internal_3, [DUMMY_ITEM, self.get_replica_from_key(2)], 3)
        ...
        with self.subTest("rank 2 internal left"):
            ...
            self._assert_internal_node_properties(internal_2_left, [DUMMY_ITEM, self.get_replica_from_key(1)], 2)
        ...
        with self.subTest("rank 2 internal right"):
            ...
            self._assert_internal_node_properties(
                internal_2_right, [self.get_replica_from_key(2), self.get_replica_from_key(key)], 2
            )
        ...
        with self.subTest("leaf 1"):
            ...
            self._assert_leaf_node_properties(leaf_1, [DUMMY_ITEM])
            self.assertIs(leaf_1.next, internal_2_left.right_subtree, "Leaf 1 should point to leaf 2 tree")

        with self.subTest("leaf 2"):
            leaf_2 = internal_2_left.right_subtree.node
            self._assert_leaf_node_properties(leaf_2, [self.item_map[1]])
            self.assertIs(leaf_2.next, internal_2_right_entries[1].left_subtree, "Leaf 2 should point to leaf 3 tree")

        with self.subTest("leaf 3"):
            leaf_3 = internal_2_right_entries[1].left_subtree.node
            self._assert_leaf_node_properties(leaf_3, [self.item_map[2]])
            self.assertIs(leaf_3.next, internal_2_right.right_subtree, "Leaf 3 should point to leaf 4 tree")
...
class TestInsertInNonEmptyTree(TestInsertInTree):
    ...
    def test_insert_internal_rank_2_left_creates_node(self):
        keys = [1, 3, 2]
        ranks = [1, 3, 2]
        item_map = {k: (self.make_item(k, f"val_{k}"), r) for k, r in zip(keys, ranks, strict=False)}
        ...
        self._assert_internal_node_properties(self.tree.node, exp_items, rank=item_map[3][1])
        ...
        self._assert_internal_node_properties(internal_l.node, items, rank=item_map[2][1])
        ...
        # Check next pointer
        self.assertIsNone(self.tree.node.next, "Root should have no next pointer")
        self.assertIsNone(internal_l.node.next, "Internal node should have no next pointer")
        self.assertIs(leaf_1.node.next, leaf_2, "Leaf 1 should point to leaf 2")
        self.assertIs(leaf_2.node.next, leaf_3, "Leaf 2 should point to leaf 3")
        self.assertIsNone(leaf_3.node.next, "Leaf 3 next pointer should be None")
    ...
    def test_insert_internal_rank_2_right_creates_node(self):
        ...
        self._assert_internal_node_properties(self.tree.node, exp_items, rank=item_map[1][1])
        ...
        self._assert_internal_node_properties(internal_r.node, items, rank=item_map[2][1])
        ...
        # Check next pointer
        self.assertIsNone(self.tree.node.next, "Root should have no next pointer")
        self.assertIsNone(internal_r.node.next, "Internal node should have no next pointer")
        self.assertIs(leaf_1.node.next, leaf_2, "Leaf 1 should point to leaf 2")
        self.assertIs(leaf_2.node.next, leaf_3, "Leaf 2 should point to leaf 3")
        self.assertIsNone(leaf_3.node.next, "Leaf 3 next pointer should be None")
    ...
    def test_insert_duplicate_key_updates_value(self):
        ...
        self.assertEqual(found_entry.item.value, 100, "'a' value should be updated to 100")
...
class TestInternalMethodsWithEntryInsert(TestInsertInTree):
    def test_insert_empty_with_entry(self):
